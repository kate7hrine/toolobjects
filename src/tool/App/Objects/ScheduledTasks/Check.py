    ...
    async def _implementation(self, i):
        ...
        for task in items.getItems():
            if task.deactivated == True:
                self.log('task {0} is deactivated'.format(task.getDbIds()))
                continue

            if task.run_at >= now or task.deactivated == True:
                self.log('task {0} will be run after {1}s'.format(task.getDbIds(), round(task.run_at.timestamp() - now.timestamp(), 2)))
    
                continue

            try:
                self.log('time for {0} ({1})'.format(task.getDbIds(), task.run_at))

                task.run(pre_i, 'scheduled_item ' + str(_iterator), self.getOption('app.scheduled_tasks.as_root'))

                _iterator += 1

                self.log('task {0} is succeed'.format(task.getDbIds()))

                if i.get('delete_after'):
                    task.delete(commit = True)

                    self.log('task {0} is deleted'.format(task.getDbIds()))
